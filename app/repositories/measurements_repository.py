...
class MeasurementsRepository:
    ...
    def get_latest(self, session_id: str) -> Optional[dict]:
        with SessionLocal() as db:
            row = (db.query(MeasurementDB)
                   .filter(MeasurementDB.session_id == session_id)
    ...
    def count(self, session_id: str) -> int:
        with SessionLocal() as db:
            return db.query(MeasurementDB).filter(MeasurementDB.session_id == session_id).count()

    def exists(self, session_id: str) -> bool:
        with SessionLocal() as db:
            return db.query(MeasurementDB.id).filter(MeasurementDB.session_id == session_id).first() is not None

    def clear_session(self, session_id: str) -> bool:
        with SessionLocal() as db:
            deleted = db.query(MeasurementDB).filter(MeasurementDB.session_id == session_id).delete()
            db.commit()
            return deleted > 0
